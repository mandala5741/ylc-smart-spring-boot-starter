...
public class Rs485XSPUtil {
	...
	/**
	 * 构建小竖屏显示指令 (0x29) - 一次性发送四行内容 协议格式：每行 = 0001 + 颜色 + 内容
	...
	public static String buildSmallScreenSmartParking(int parkingSpaceCount) {
		String[] lines = { "智慧停车", "车位" + parkingSpaceCount + "个", "一车一杆", "请勿跟车" };
	...
	public static String buildSmallScreenEntry(String plateNumber, int parkingSpaceCount) {
		String[] lines = { "欢迎光临", plateNumber, "一车一杆自动识别", "剩余车位" + parkingSpaceCount + "个" };
		int[] colors = { 0x08, 0x08, 0x08, 0x08 };
		return buildSmallScreenDisplayCommand(lines, colors);
	}
	...
	public static String buildSmallScreenExit(String plateNumber, String amount) {
		String[] lines = { "一路平安", plateNumber, "缴费" + amount + "元", "谢谢光临" };
		int[] colors = { 0x08, 0x08, 0x08, 0x08 };
		return buildSmallScreenDisplayCommand(lines, colors);
	}
	...
	public static String[] buildSmallScreenEntryScene(String plateNumber, int parkingSpaceCount) {
		return new String[] { buildSmallScreenEntry(plateNumber, parkingSpaceCount), // 显示
	...
	public static String[] buildSmallScreenExitScene(String plateNumber, int amount) {
		return new String[] { buildSmallScreenExit(plateNumber, String.valueOf(amount)), // 显示
	...
	public static String buildRs485Json(String[] commands) {
		...
		for (int i = 0; i < commands.length; i++) {
			rs485Data.append(String.format("{\"encodetype\":\"hex2string\",\"data\":\"%s\"}", commands[i]));
	 ...
	 * @return JSON格式字符串
	 */
	public static String buildRs485Json(String[] commands, String gpioAction, String gpioNum, String errorStr,
			int errorNum) {
		StringBuilder rs485Data = new StringBuilder();
		rs485Data.append("[");
	...
	/**
	 * 构建显示指令 (0x37) - 固定显示内容 协议格式：控制字1(行号) | 控制字2(00) | 控制字3(颜色) | 控制字4(00) | 内容
	 */
	...
	public static String[] buildSmartParkingDisplay(int parkingSpaceCount) {
		return new String[] { buildDisplayCommand(1, 0, "智慧停车"),
				buildDisplayCommand(2, 3, "车位" + parkingSpaceCount + "个"), buildDisplayCommand(3, 4, "一车一杆"),
	...
	public static String[] buildWelcomeDisplay() {
		return new String[] { buildDisplayCommand(1, 0, "欢迎光临"), buildDisplayCommand(2, 3, "车牌识别"),
				buildDisplayCommand(3, 4, "一车一杆"), buildDisplayCommand(4, 8, "减速慢行") };
	}
	...
	public static String[] buildEntryDisplay(String plateNumber, int parkingSpaceCount) {
		return new String[] { buildDisplayCommand(1, 0, "欢迎光临"), buildDisplayCommand(2, 3, plateNumber),
				buildDisplayCommand(3, 4, "一车一杆自动识别"), buildDisplayCommand(4, 8, "剩余车位" + parkingSpaceCount + "个") };
	}
	...
	public static String[] buildExitDisplay(String plateNumber, String amount) {
		return new String[] { buildDisplayCommand(1, 0, "一路平安"), buildDisplayCommand(2, 3, plateNumber),
				buildDisplayCommand(3, 4, "缴费" + amount + "元"), buildDisplayCommand(4, 8, "谢谢光临") };
	}
	...
	public static String[] buildEntryScene(String plateNumber, int parkingSpaceCount) {
		return new String[] {
				// 显示内容
				buildDisplayCommand(1, 0, "欢迎光临"), buildDisplayCommand(2, 3, plateNumber),
				buildDisplayCommand(3, 4, "一车一杆自动识别"), buildDisplayCommand(4, 8, "剩余车位" + parkingSpaceCount + "个"),
				// 语音播报 - 使用字符索引
	...
	public static String[] buildExitScene(String plateNumber, int amount) {
		return new String[] {
				// 显示内容
				buildDisplayCommand(1, 0, "一路平安"), buildDisplayCommand(2, 3, plateNumber),
				buildDisplayCommand(3, 4, "缴费" + amount + "元"), buildDisplayCommand(4, 8, "谢谢光临"),
				// 语音播报 - 使用字符索引
