...
public class Rs485Util {
    ...
    public static String buildExitBlessingVoice(String plateNumber) {
        // 组合：车牌语音 + 一路顺风 + 欢迎再次光临
        int[] voiceIndexes = {
                getPlateVoiceIndex(plateNumber),  // 根据车牌获取对应语音
    ...
    /**
     * 根据车牌号获取对应的语音索引
     * 注意：标准语音中车牌需要预定义，这里使用"此车"作为通用车牌语音
     */
    ...
    /**
     * 构建下发临显内容指令 (0x27) - 临时显示内容，可设置时长
     * 协议格式：控制字1(行号) | 控制字2(时长) | 控制字3(颜色) | 控制字4(保留) | 内容
     *
    ...
    /**
     * 构建加载广告内容指令 (0x25) - 固定广告内容，不滑动显示
     * 协议格式：控制字1(行号) | 控制字2(颜色) | 控制字3(保留) | 内容
     *
     * @param line    行号 (1-4)
     * @param color   显示颜色 (1=红, 2=绿, 3=黄)
     * @param content 显示内容
    ...
    public static String[] buildEntryScene(String plateNumber, int parkingSpace) {
        return new String[] {
                buildLoadAdCommand(1, COLOR_RED, "欢迎光临"),
                buildLoadAdCommand(2, COLOR_RED,plateNumber),
                buildLoadAdCommand(3, COLOR_RED,"一车一杆自动识别"),
                buildLoadAdCommand(4, COLOR_RED,"减速慢行"),
                buildParkingSpaceCommand(parkingSpace, 60, COLOR_GREEN),
                buildPlayVoiceCommand(plateNumber + "\u0001") // 车牌 + 欢迎光临语音
        };
    }
    ...
    public static String[] buildExitScene(String plateNumber, String amount) {
        return new String[] {
                buildLoadAdCommand(1, COLOR_RED,"一路平安"),
                buildLoadAdCommand(2, COLOR_RED, plateNumber),
                buildLoadAdCommand(3, COLOR_RED,"请交费" + amount + "元"),
                buildLoadAdCommand(4, COLOR_RED,"谢谢光临"),
    ...
    public static String[] buildFixedDisplay(String line1, String line2, String line3, String line4, int color) {
        return new String[] {
                buildLoadAdFixedDisplayCommand(1, color, line1),
                buildLoadAdFixedDisplayCommand(2, color, line2),
                buildLoadAdFixedDisplayCommand(3, color, line3),
    ...
    public static String[] buildFixedParkingInfoDisplay(int availableSpaces, int totalSpaces) {
        return buildFixedDisplay(
                "停车场信息",
                "总车位:" + totalSpaces + " 剩余:" + availableSpaces,
                "请有序停车",
                "谢谢配合",
                COLOR_GREEN
        );
    ...
    public static void main(String[] args) {
        ...
        /**
        // 1. 默认固定欢迎显示
        String[] welcomeDisplay = buildFixedWelcomeDisplay();
        System.out.println("固定欢迎显示:");
        for (String cmd : welcomeDisplay) {
            System.out.println(cmd);
        }**/
        /**
        // 4. 生成临时网址二维码（显示30秒）
        String tempUrlQrcode = Rs485Util.buildTempQrcode("https://www.example.com", 30);
        ...
        **/
        ...
        String line3 = buildAutoRecognitionLine();
        String line4 = buildSlowDownLine();
        String voice = buildPlateVoiceCommand(plateNumber);
        ...
        // 测试完整场景
        String[] entryScene = buildEntryScene(plateNumber, 56);
        System.out.println("\n入场场景:");
        for (String cmd : entryScene) {
            System.out.println(cmd);
        }
        String[] entryScenes = buildExitScene(plateNumber, "10");
        System.out.println("\n出场场景:");
        for (String cmd : entryScenes) {
            System.out.println(cmd);
        }
