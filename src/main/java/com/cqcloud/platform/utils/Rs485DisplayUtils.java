...
public class Rs485DisplayUtils {
    ...
    public static String buildPaySuccessVoice(String plateNumber) {
        // 组合：车牌语音 + 缴费成功 + 谢谢
        int[] voiceIndexes = {
                getPlateVoiceIndex(plateNumber),  // 车牌语音
    ...
    public static String[] buildSmartParkingDisplay(int parkingSpaceCount, int color) {
        return new String[] {
                buildLoadTempDisplayCommand(1, color, "智慧停车"),
                buildLoadTempDisplayCommand(2, color, "车位" + parkingSpaceCount + "个"),
                buildLoadTempDisplayCommand(3, color, "一车一杆"),
                buildLoadTempDisplayCommand(4, color, "请勿跟车")
        };
    }
    ...
    public static String[] buildEntryWelcomeDisplay(String plateNumber, int parkingSpaceCount) {
        return new String[] {
                buildLoadTempDisplayCommand(1, COLOR_GREEN, "欢迎光临"),
    ...
    public static String[] buildExitDisplay(String plateNumber, String amount) {
        return new String[] {
                buildLoadTempDisplayCommand(1, COLOR_GREEN, "一路平安"),
                buildLoadTempDisplayCommand(2, COLOR_GREEN, plateNumber),
    ...
    public static String[] buildParkingFullWarning(int parkingSpaceCount) {
        ...
        return new String[] {
                buildLoadTempDisplayCommand(1, color, warningText),
                buildLoadTempDisplayCommand(2, color, "剩余" + parkingSpaceCount + "个"),
                buildLoadTempDisplayCommand(3, color, "请尽快停车"),
                buildLoadTempDisplayCommand(4, color, "谢谢配合")
        };
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println("智慧停车四行显示命令:");
        String[] commands = buildSmartParkingDisplay(parkingSpaceCount);
        for (int i = 0; i < commands.length; i++) {
            System.out.println("第" + (i + 1) + "行: " + commands[i]);
        }

        System.out.println("\n显示效果:");
        System.out.println("智慧停车");
        System.out.println("车位" + parkingSpaceCount + "个");
        System.out.println("一车一杆");
        System.out.println("请勿跟车");

        // 测试不同车位数的显示
        System.out.println("\n不同车位数的显示:");
        int[] testCounts = {5, 15, 0, 48};
        for (int count : testCounts) {
            System.out.println("\n车位" + count + "个时的显示:");
            String[] testCommands = buildSmartParkingDisplay(count);
        ...
        // 测试取消显示
        String cancelCmd = buildCancelAllTempDisplayCommand();
        System.out.println("\n取消所有显示: " + cancelCmd);


        String a =Rs485Util.buildPlayVoiceCommand("渝BS775W" + "\u0001"); // 车牌 + 欢迎光临语音
        System.out.println("\n取消所有显示1: " + a);**/
